  ...
  for(;whIt!=whEnd;++whIt) {
	...
	for(int isl=1;isl<4;isl++){
	  ...
	  if(dodraw){
	    ...
	    DigiXSecMBSlLayer[iw][isec-1][ich-1][isl-1][0]->SetLineColor(1);
	...
	for(int isl=1;isl<4;isl++){
	  ...
	  if(dodraw){
	    // look first for the maximum bin entries
	    TBoxSecMBSlLayer[iw][isec-1][ich-1][isl-1][0]->SetLineColor(1);
	...
	for(int isl=1;isl<4;isl++){
	  ...
	  if(dodraw){
	    // look first for the maximum bin entries
	    TBoxSegSecMBSlLayer[iw][isec-1][ich-1][isl-1][0]->SetLineColor(1);
	...
	for(int isl=1;isl<4;isl++){
      ...
      //===================================================================================
      //    Time Segment - From Segment T0: 
      //===================================================================================
      edm::LogVerbatim ("DTDPGSummary") << "[DTDPGCreateAnalyzerSummary]: Drawing Time Segments wh " 
					<< (iw-2) << " sec " << isec << endl;
      bool TsegDraw=false;
      TCanvas *  TimeSegC = new TCanvas("TimeSegC", "",201,81,999,699);
      TimeSegC->Divide(2,2);
      ...
      for(int ich=ichmin;ich<5;ich++){
       ...
       stringstream histoNameTimeSeg;
       histoNameTimeSeg << "hTimeSeg_" << Whname[iw]  << "_S" << isec  << "_MB" << ich;
       TH1F *histoTimeSeg = getHisto<TH1F>(histoNameTimeSeg.str());
       if(histoTimeSeg != NULL)
       {
         TsegDraw=true;
         TimeSegC->cd(ich);
         ...
         try {
          ...
          TimSegMean = fTimSeg.GetParameter(1);
            ...
            edm::LogError("DTDPGSummary") << "[DTDPGCreateAnalyzerSummary]: Error fitting TimeSegment for Wheel " << iw
                                       <<" Sector " << isec << " Station " << ich;
         }
         ...
         L1.SetTextColor(4);
         char MBstation[10];
         sprintf(MBstation,"MB%d", ich);
         L1.DrawText(-190.0,0.02,MBstation);
       }
      }//end loop chambers
      if(TsegDraw)
          createGifFile("TimeSegment",(iw-2),isec,"",TimeSegC);
	delete TimeSegC;
      gStyle->SetOptStat(10);
      gStyle->SetStatW(0.35);
	...
	for(int il=1;il<5;il++)
     ...
     if(ich==4 && isl !=2)
     {
       if(isec ==4 || isec ==10)
       {
        ...
        ExtraS13S14->cd(iExtraPad); iExtraPad++;
	  DigiXSecMBSlLayer[iw][isec_extra-1][ich-1][isl-1][0]->SetLineColor(1);
...
// Time Segments
//
   edm::LogVerbatim ("DTDPGSummary") << "[DTDPGCreateAnalyzerSummary]: Drawing Time Segments wh " << (iw-2)<< endl;
   bool TsegWDraw=false;
   TCanvas *  TimeSegW = new TCanvas("TimeSegW", "Time of Seg (BX)",201,81,999,699);
   TimeSegW->Divide(6,2);

/*
   TCanvas *  TimeSegW = new TCanvas("TimeSegW", "Time of Seg (BX)",500,0,900,950);
   TimeSegW->SetFillColor(0);
   TimeSegW->SetTopMargin(0.45);
   TimeSegW->SetBottomMargin(0.25);
   TimeSegW->SetRightMargin(0.12);
   TimeSegW->Divide(1,5,0.,0);
*/


   for(int isec=1; isec<13; isec++)
   //for(int isec=12; isec<13; isec++)
   {
     TimeSegW->cd(isec);
     TH1F *histoTimeSeg[4];
     int theMax=0;
     bool TsegWDraw2=false;
     int ichmin=1; if (isec==13|| isec==14)ichmin=4;
     for(int ich=ichmin;ich<5;ich++){
       stringstream histoNameTimeSeg;
       histoNameTimeSeg << "hTimeSeg_" << Whname[iw]  << "_S" << isec  << "_MB" << ich;
       histoTimeSeg[ich-1] = getHisto<TH1F>(histoNameTimeSeg.str());
       if(histoTimeSeg[ich-1] != NULL)
       {
         TsegWDraw=true;
         TsegWDraw2=true;
         int max=histoTimeSeg[ich-1]->GetMaximum();
         if (max > theMax || ich==ichmin){ 
             first_to_paint=histoTimeSeg[ich-1];
             theMax=max;
         }
       }
     }//end loop chambers
     if(TsegWDraw2)
     {
        first_to_paint->GetYaxis()->SetLabelSize(0.09);
        first_to_paint->GetXaxis()->SetLabelSize(0.09);
        first_to_paint->GetXaxis()->SetLabelOffset(-0.025);
        first_to_paint->GetXaxis()->SetTitle("ns");
        first_to_paint->GetXaxis()->SetTitleSize(0.09);
        first_to_paint->GetXaxis()->SetTitleOffset(0.5);
        first_to_paint->GetXaxis()->SetNdivisions(604);
        stringstream htitle; htitle << "Time Segment W" << (iw-2) << " S" << isec;
        first_to_paint->SetTitle(htitle.str().c_str());

        first_to_paint->Draw("histo");
        for(int ich=0;ich<4;ich++)
        if(histoTimeSeg[ich]){histoTimeSeg[ich]->SetLineColor(ich+1); histoTimeSeg[ich]->Draw("histosame");}
      }
       
   }// end loop sectors

   if(TsegWDraw)
     createGifFile("TimeSegment",(iw-2),"",TimeSegW);
...
//   first_to_paint_MB[ic-1]->SetMinimum(ymin*.20);
 ...
 for(int ic=1;ic<5;ic++)
 {
    ...
    OccupanALL->cd(ic);
    first_to_paint_MB[ic-1]->Draw("histo");
    ...
    for(int iw=0;iw<5;iw++)
    {
      ...
      OccupanALL->cd(ic);  OccupMB[iw][ic-1]->Draw("histosame");
    }
 }
...
// first_to_paint->GetYaxis()->SetTitleOffset(0.5);
 ...
 for(int ic=0;ic<4;ic++)
 {
   ...
   {
      float nbmax0[4]={0,0,0,0};
      if(TBoxSegMB[iw][ic]!=NULL)         nbmax0[0]=TBoxSegMB[iw][ic]->GetMaximum();
      if(TBoxSegMBTop[iw][ic]!=NULL)      nbmax0[1]=TBoxSegMBTop[iw][ic]->GetMaximum();
      if(TBoxSegMBBottom[iw][ic]!=NULL)   nbmax0[2]=TBoxSegMBBottom[iw][ic]->GetMaximum();
      if(TBoxSegMBVertical[iw][ic]!=NULL) nbmax0[3]=TBoxSegMBVertical[iw][ic]->GetMaximum();
...
// Summary Time Segments
  ...
  for(int ic=0;ic<4;ic++)
  {
    stringstream hname; hname << "TimeSegAv_" << Whname[iw] << "_MB" << ic+1 ;
    //stringstream htitle; htitle << "Time Segment (BX) " << Whname[iw] << " MB" << ic+1 ;
    stringstream htitle; htitle << "Time Segment (ns) " << Whname[iw] << " MB" << ic+1 ;
    TimeSegAv[iw][ic]= new TH1F(hname.str().c_str(),htitle.str().c_str(),14,1.,15.);
    for(int ib=1; ib<15; ib++)
    {
       stringstream iblab; iblab << "Sect-"<<ib;
       TimeSegAv[iw][ic]->GetXaxis()->SetBinLabel(ib,iblab.str().c_str());
    }
  }
    ...
    for(int ich=ichmin;ich<5;ich++){
     ...
     if(histoTimeSeg != NULL)
     {
       ...
       try {
        ...
        histoTimeSeg->Fit(&fTimSeg,"CQO","",TimSegMean-30,TimSegMean+30);
        TimSegMean = fTimSeg.GetParameter(1);
        TimSegSigma = fTimSeg.GetParameter(2);
           
        //TimeSegAv[iw][ich-1]->SetBinContent(isec,TimSegMean/25.0); // Units of BX 
        //TimeSegAv[iw][ich-1]->SetBinError(isec,TimSegSigma/25.0); // Units of  BX 
        TimeSegAv[iw][ich-1]->SetBinContent(isec,TimSegMean); // Units of ns 
        TimeSegAv[iw][ich-1]->SetBinError(isec,TimSegSigma); // Units of  ns 
  ...
  }//end loop Wheel & Sectors 

  
  //TCanvas * TimeSegA=new TCanvas("TimeSegA","Time of Segment (BX)",500,0,900,950);
  TCanvas * TimeSegA=new TCanvas("TimeSegA","Time of Segment (ns)",500,0,900,950);
  TimeSegA->SetFillColor(0);
  TimeSegA->SetTopMargin(0.45);
  TimeSegA->SetBottomMargin(0.25);
  TimeSegA->SetRightMargin(0.12);
  TimeSegA->Divide(1,5,0.,0);
  for (int iw=1;iw<6;++iw) {
    TimeSegA->cd(iw)->SetFillColor(0);
    TimeSegA->cd(iw)->SetGrid();
    TimeSegA->cd(iw)->SetBorderMode(0);
  }
 ...
 for (int iw=0;iw<5;++iw) {
   ...
   TimeSegA->cd(iw+1);
   TimeSegA->cd(iw+1)->SetGrid();

   for (int ist=0;ist<4;++ist) {
     string opt = ist ? "same" : "";
     TimeSegAv[iw][ist]->SetLineWidth(2);
     TimeSegAv[iw][ist]->SetLineWidth(2);
     TimeSegAv[iw][ist]->SetLineColor(ist+1);
     //TimeSegAv[iw][ist]->GetYaxis()->SetRangeUser(-10,10);  // in BX
          TimeSegAv[iw][ist]->GetYaxis()->SetRangeUser(-200,200);    // in ns
     TimeSegAv[iw][ist]->GetYaxis()->SetLabelSize(0.12);
     TimeSegAv[iw][ist]->GetYaxis()->SetNdivisions(504);
     TimeSegAv[iw][ist]->GetXaxis()->SetTitle("sector");
     TimeSegAv[iw][ist]->GetXaxis()->SetTitleSize(0.12);
     TimeSegAv[iw][ist]->GetXaxis()->SetLabelSize(0.12);
     TimeSegAv[iw][ist]->GetXaxis()->CenterLabels();
     TimeSegAv[iw][ist]->GetXaxis()->SetNdivisions(14);
     TimeSegAv[iw][ist]->Draw(opt.c_str());
     if (!ist) wheelName[iw]->Draw();
   }
    //TPaveLabel * TimeSegATitle = new TPaveLabel(0.1,0.93,0.85,0.99,"Time Segment (BXs)");
    TPaveLabel * TimeSegATitle = new TPaveLabel(0.1,0.93,0.85,0.99,"Time Segment (ns)");
    TimeSegATitle->SetFillColor(0);
    TimeSegATitle->SetTextColor(4);
    TimeSegA->cd();
    TimeSegATitle->Draw();
 }


  createGifFile("TimeSegment","",TimeSegA,false);
  delete TimeSegA;
