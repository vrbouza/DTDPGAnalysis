...
DTEffOfflineAnalyzer::DTEffOfflineAnalyzer(const ParameterSet& pset) {
  ...
  theRecHits2DToken_ = consumes<DTRecSegment2DCollection>(
        edm::InputTag(pset.getParameter<string>("recHits2DLabel")));

  // the name of the 4D rec hits collection
  //theRecHits4DLabel = pset.getParameter<string>("recHits4DLabel");
  theRecHits4DToken_ = consumes<DTRecSegment4DCollection>(
