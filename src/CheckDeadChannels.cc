...
CheckDeadChannels::~CheckDeadChannels(){
  ...
  std::ifstream  refRun ("/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DT/OfflineCode/Scripts/referenceRun.txt");
  ...
  std::ifstream  DeadChannelList (deadname);
  std::ifstream  DeadChannelRef  (deadnameRef.c_str());
  cout<<" opened "<<deadname<<" and "<<deadnameRef<<endl;

  //DeadComp = new ofstream(comparison); // Problems in version CMSSW_9_0_0
  DeadComp.open(comparison);
