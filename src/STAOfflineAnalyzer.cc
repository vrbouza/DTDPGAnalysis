...
STAOfflineAnalyzer::STAOfflineAnalyzer(const ParameterSet& pset) : _ev(0){
  ...
  theRecHits1DToken_ = consumes<DTRecHitCollection>(edm::InputTag(theRecHits1DLabel));

  // the name of the 2D rec hits collection
  theRecHits2DLabel = pset.getParameter<string>("recHits2DLabel");
  theRecHits2DToken_ = consumes<DTRecSegment2DCollection>(edm::InputTag(theRecHits2DLabel));

  // the name of the 4D rec hits collection
  theRecHits4DLabel = pset.getParameter<string>("recHits4DLabel");
  theRecHits4DToken_ = consumes<DTRecSegment4DCollection>(edm::InputTag(theRecHits4DLabel));

  // the name of the STA rec hits collection
  theSTAMuonLabel = pset.getParameter<string>("SALabel");
  theSTAMuonToken_ = consumes<reco::TrackCollection>(edm::InputTag(theSTAMuonLabel));
...
void STAOfflineAnalyzer::analyzeSATrack(const Event & event,
  ...
  //event.getByLabel(theRecHits2DLabel, segs2d);  // Doesn't work after 75X
  event.getByToken(theRecHits2DToken_, segs2d);
  ...
  for (staTrack = staTracks->begin(); staTrack != staTracks->end(); ++staTrack){
    ...
    //event.getByLabel(theRecHits1DLabel, hits1d);  // Doesn't work after 75X
    event.getByToken(theRecHits1DToken_, hits1d);
