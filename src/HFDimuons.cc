...
HFDimuons::HFDimuons(const edm::ParameterSet& iConfig) :
  ...
  fTracksToken_ = consumes<reco::Track>(InputTag(fTracksLabel)); 
  fPrimaryVertexToken_ = consumes<VertexCollection>(InputTag(fPrimaryVertexLabel));
  fMuonsToken_= consumes<MuonCollection>(InputTag(fMuonsLabel));
...
void HFDimuons::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  //iEvent.getByLabel(fPrimaryVertexLabel, recoPrimaryVertexCollection);  Doesn't work after 75X
  iEvent.getByToken(fPrimaryVertexToken_, recoPrimaryVertexCollection);  
  ...
  //iEvent.getByLabel(fMuonsLabel, hMuons); Doesn't work after 75X
  iEvent.getByToken(fMuonsToken_, hMuons);
  if (!hMuons.isValid()) {
  ...
  //iEvent.getByLabel(fTracksLabel, hTracks);Doesn't work after 75X
  iEvent.getByToken(fTracksToken_, hTracks);
  if(!hTracks.isValid()) {
