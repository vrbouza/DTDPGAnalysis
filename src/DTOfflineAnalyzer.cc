...
DTOfflineAnalyzer::DTOfflineAnalyzer(const ParameterSet& pset) : _ev(0){
  ...
  theRecHits1DToken_ = consumes<DTRecHitCollection>(string(theRecHits1DLabel));

  // the name of the 2D rec hits collection
  theRecHits2DLabel = pset.getParameter<string>("recHits2DLabel");
  theRecHits2DToken_ = consumes<DTRecSegment2DCollection>(string(theRecHits2DLabel));

  // the name of the 4D rec hits collection
  theRecHits4DLabel = pset.getParameter<string>("recHits4DLabel");
  theRecHits4DToken_ = consumes<DTRecSegment4DCollection>(string(theRecHits4DLabel));

  // the name of the 4D rec hits collection
  theSTAMuonLabel = pset.getParameter<string>("SALabel");
  theSTAMuonToken_ = consumes<reco::TrackCollection>(string(theSTAMuonLabel));
...
//    time in track segments
     ...
     if(doTrig)
     {
      stringstream hNameTrigg1,  hNameTrigg2,  hNameTrigg3,  hNameTrigg4;
      stringstream hNameTrigg11,  hNameTrigg12,  hNameTrigg13,  hNameTrigg14;
      stringstream hTitleTrigg1,  hTitleTrigg2,  hTitleTrigg3,  hTitleTrigg4;
      stringstream hTitleTrigg11,  hTitleTrigg12,  hTitleTrigg13,  hTitleTrigg14;

      hNameTrigg1  << "hDDU_LocalTrackPos_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg1 << "Best Local Segment Position,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg1.str(),hTitleTrigg1.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg2  << "hDDU_LocalTrackPosandTrig_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg2 << "Best Local Segment Position with Trigger,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg2.str(),hTitleTrigg2.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg3  << "hDDU_LocalTrackPosHHHL_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg3 << "Best Local HHHL Segment Position,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg3.str(),hTitleTrigg3.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg4  << "hDDU_LocalTrackPosHHHLandTrig_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg4 << "Best Local HHHL Segment Position with Trigger,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg4.str(),hTitleTrigg4.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg11  << "hDDU_LocalTrackThetaPos_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg11 << "Best Local Theta Segment Position,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg11.str(),hTitleTrigg11.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg12  << "hDDU_LocalTrackThetaPosandTrig_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg12 << "Best Local Theta Segment Position with Trigger,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg12.str(),hTitleTrigg12.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg13  << "hDDU_LocalTrackThetaPosH_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg13 << "Best Local H Theta Segment Position,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg13.str(),hTitleTrigg13.str(),"",21,-105.,105.); // *** histos:  **********

      hNameTrigg14  << "hDDU_LocalTrackThetaPosHandTrig_"  << Whname[iw] << "_S" << sect << "_MB" << jst ;
      hTitleTrigg14 << "Best Local H Theta Segment Position with Trigger,"  << Whname[iw] << "S" << sect << "/MB" << jst ;
      createTH1F(hNameTrigg14.str(),hTitleTrigg14.str(),"",21,-105.,105.); // *** histos:  **********
...
//  get the tracking geometry
  ...
  //event.getByLabel(theRecHits1DLabel, dtRecHits);  // Doesn't work after 75X
  event.getByToken(theRecHits1DToken_, dtRecHits);
...
// ==========================================================================================================
  ...
  //if (!mc) event.getByLabel(theDTLocalTriggerLabel, digis);  // Doesn't work after 75X
  if (!mc) event.getByToken(theDTLocalTriggerToken_, digis);

  // Get the 4D rechit collection from the event -------------------
  edm::Handle<DTRecSegment4DCollection> segs;
  //event.getByLabel(theRecHits4DLabel, segs);   // Doesn't work after 75X
  event.getByToken(theRecHits4DToken_, segs);
...
//=======================================
  ...
  //event.getByLabel(theDTLocalTriggerLabel, allLocalTriggers);   // Doesn't work after 75X
  event.getByToken(theDTLocalTriggerToken_, allLocalTriggers); 
...
//  loop over triggers of this chamber
    for (DTLocalTriggerCollection::const_iterator trigtrack = range.first;trigtrack!=range.second;++trigtrack){
      ...
      if(trigtrack->trTheta()>0) // ThetaTrigger 
         if(trigtrack->trTheta() > thetaqual[iw][SCst-1][SCsect-1])thetaqual[iw][SCst-1][SCsect-1]=trigtrack->trTheta();
...
// loop on chambers ==============================================
  ...
  for (std::vector<const DTChamber*>::const_iterator ch = chs.begin();
       ch!=chs.end() ; ++ch) {
...
// ********** loop on 4D segments *********************************
 ...
 //   int iwheel = seg->chamberId().wheel() + 3;  // Wheel -2,.+2  => 1,...5
      ...
      if (fillz) histo(histoobj2.str())->Fill(zglobal);     
...
//      cout <<  -atan(glbDir.x()/ glbDir.y()) << endl;
      ...
      if (phiSeg) {
        ...
        if(doTrig)
        {// Local Trigger Eff (for Single chamber commissioning purposes
          if (phiHits.size()>3 && abs(phiLocal) < 40. )
          {
            stringstream hNameTrigg1; 
            hNameTrigg1  << "hDDU_LocalTrackPos_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
            histo(hNameTrigg1.str())->Fill((*seg).localPosition().x()); 
            if(qual[iw][ist-1][iisect-1] > -1 ){  // AnyQual 
              stringstream hNameTrigg2; 
              hNameTrigg2  << "hDDU_LocalTrackPosandTrig_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
              histo(hNameTrigg2.str())->Fill((*seg).localPosition().x()); 
            }
          }
          if (phiHits.size()>6 && abs(phiLocal) < 40. )
          {
            stringstream hNameTrigg1; 
            hNameTrigg1  << "hDDU_LocalTrackPosHHHL_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
            histo(hNameTrigg1.str())->Fill((*seg).localPosition().x()); 
            if(qual[iw][ist-1][iisect-1] > 4 ){ //HH or HL
              stringstream hNameTrigg2; 
              hNameTrigg2  << "hDDU_LocalTrackPosHHHLandTrig_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
              histo(hNameTrigg2.str())->Fill((*seg).localPosition().x()); 
            }
          }
        }// ======= End Local Phi Trigger Eff if doTrig
...
// ===========================================================
      ...
      if (zedSeg) {
        ...
        {// Local Trigger Eff (for Single chamber commissioning purposes
          ...
          {
            stringstream hNameTrigg1; 
            hNameTrigg1  << "hDDU_LocalTrackThetaPos_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
            histo(hNameTrigg1.str())->Fill((*seg).localPosition().y()); 
            if(thetaqual[iw][ist-1][iisect-1] > 0 ){  // AnyQual 
              stringstream hNameTrigg2; 
              hNameTrigg2  << "hDDU_LocalTrackThetaPosandTrig_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
              histo(hNameTrigg2.str())->Fill((*seg).localPosition().y()); 
            }
          }
          if (zedHits.size()>3)
          {
            stringstream hNameTrigg1; 
            hNameTrigg1  << "hDDU_LocalTrackThetaPosH_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
            histo(hNameTrigg1.str())->Fill((*seg).localPosition().y()); 
            if(thetaqual[iw][ist-1][iisect-1] ==3 ){ //H
              stringstream hNameTrigg2; 
              hNameTrigg2  << "hDDU_LocalTrackThetaPosHandTrig_"  << Whname[iw] << "_S" << iisect << "_MB" << ist ;
              histo(hNameTrigg2.str())->Fill((*seg).localPosition().y()); 
            }
          }
        }// ======= End Local Phi Trigger Eff if doTrig
...
//  const std::vector<DTChamber*> & chs = dtGeom->chambers();
  ...
  //event.getByLabel(theSTAMuonLabel, staTracks);   // Doesn't work after 75X
...
//  loop on associated rechits ----------------------------------------------------------
    ...
    for(trackingRecHit_iterator recHit = rhbegin; recHit != rhend; ++recHit){
     ...
     //GlobalPoint gpos=geomDet->toGlobal((*recHit)->localPosition()); // Moved to inside IF is from DT, RPC hits give problems sometimes
